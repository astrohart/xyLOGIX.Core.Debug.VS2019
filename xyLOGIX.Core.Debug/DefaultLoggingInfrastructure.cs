...
namespace xyLOGIX.Core.Debug
{
   ...
   public class DefaultLoggingInfrastructure : ILoggingInfrastructure
   {
      ...
      public virtual void DeleteLogIfExists()
      {
         ...
         {
            // If we cannot write to the folder where the log file to be deleted
            // sits in, then Heaven help us! However the software should try to
            // work at all costs, so this method should just silently fail in
            // this case.
            DebugUtils.WriteLine(
                DebugLevel.Error,
      ...
      /// cref="P:log4net.Appender.FileAppender.File"/> property from the first
      /// appender in the list of appenders that is of type <see cref="T:log4net.Appender.FileAppender"/>.
      /// </summary>
      /// <returns>
      ...
      /// Set to <c>true</c> to suppress the display of logging messages to the
      /// console if a log file is being used. If a log file is not used, then
      /// no logging at all will occur if this parameter is set to <c>true</c>.
      /// </param>
      /// <param name="repository">
      /// (Optional.) Reference to an instance of an object that implements the
      /// <see cref="T:log4net.Repository.ILoggerRepository"/> interface. Supply
          ...
          ILoggerRepository repository = null)
      {
         ...
         // This is handy in the case where the user does not have write access
         // to the C:\ProgramData directory, for example.
         ...
         // path indicated. If the configuration file pathname is blank and/or
         // it does not exist at the path indicated, then call the version of
         // XmlConfigurator.Configure that does not take any arguments. On the
         // other hand, if the configurationFilePathname parameter is not blank,
         // and it specifies a file that actually does exist at the specified
         // path, then pass that path to the XmlConfigurator.Configure method.
         if (string.IsNullOrWhiteSpace(configurationFilePathname) ||
             !File.Exists(configurationFilePathname))
         {
            // If the file specified by the configurationFilePathname does not
            // actually exist, the author of this software needs to know about
            // it, so throw a FileNotFoundException
      ...
      /// True to activate the functionality of writing to a log file; false to
      /// suppress. Usually used with the <see cref="consoleOnly"/> parameter
      /// set to true.
      /// </param>
          ...
          bool muteConsole = false)
      {
         ...
         // do not print anything in this method if verbosity is set to anything
         // less than 2
          ...
          ILoggerRepository repository)
      {
         ...
         // writeable. If we can't get write access to the log file directory,
         ...
         // Set options on the file appender of the logging system to minimize
         // locking issues
