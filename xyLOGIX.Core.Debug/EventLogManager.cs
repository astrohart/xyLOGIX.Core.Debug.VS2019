...
namespace xyLOGIX.Core.Debug
{
    ...
    internal class EventLogManager : IEventLogManager
    {
        ...
        /// </summary>
        private static IEventLogTypeValidator EventLogTypeValidator { [DebuggerStepThrough] get; } =
        ...
        public bool IsInitialized
        {
            ...
            get
            {
                ...
                try
                {
                    ...
                    result = !EventLogType.None.Equals(Type) && !EventLogType.Unknown.Equals(Type);
                }
                catch (Exception ex)
                {
        ...
        /// content of the logging message is specified by the <paramref name="content" />
        /// parameter.
        /// </summary>
        ...
        /// </returns>
        public bool Initialize([NotLogged] string eventSourceName, EventLogType logType)
        {
            var result = false;
            ...
            try
            {
                ...
                // Check whether the value of the parameter, 'eventSourceName', is blank. If this is
                ...
                {
                    // The parameter, 'eventSourceName' was either passed a null value, or it is
                ...
                // Check to see whether the type of event log is within the defined value set. If
                ...
                {
                    // The type of event log is NOT within the defined value set.  This is not
                    // desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                // 'eventSourceName'.  If this is not the case, then write an error message to the
                // Debug log and then terminate the execution of this method.
                if (!EventLog.SourceExists(eventSourceName))
                {
                    // No Event Source having the name 'eventSourceName' is currently configured.
                    // Attempt to create such a source.
                    System.Diagnostics.Debug.WriteLine(
                        $"*** WARNING: No Event Source having the name '{eventSourceName}' is currently configured.  Attempting to create such a source..."
                    );

                    EventLog.CreateEventSource(eventSourceName, logType.ToString());
                ...
                // Finally, save the event quote and type settings in the Source and Type
                // properties.
            ...
            {
                // If an exception was caught, de-initialize the member prevent this class from
                // working in future calls.
            ...
            System.Diagnostics.Debug.WriteLine($"EventLogManager.Initialize: Result = {result}");
