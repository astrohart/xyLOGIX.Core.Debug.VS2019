...
namespace xyLOGIX.Core.Debug
{
    ...
    /// Ensures dependent assemblies (e.g., log4net, PostSharp runtime) are found in
    /// the VSIX folder even if the host didn't add a binding path.
    /// </summary>
    [Log(AttributeExclude = true)]
    internal static class VsixHosting
    {
        /// <summary>Value that indicates whether the assembly resolver has been installed.</summary>
        ...
        internal static void EnsureAssemblyResolver()
        {
            try
            {
                ...
                // Check to see whether the assembly resolver has already been installed. Otherwise,
                // write an error message to the log file, and then terminate the execution of this
                // method.
                if (Interlocked.CompareExchange(ref _resolverInstalled, 0, 0) != 0)
                {
                ...
                {
                    // The variable, baseDir, has a null reference for its value, or it is blank.
                ...
                // Check to see whether the folder, 'baseDir', exists on the file system. If this is
                // not the case, then write an error message to the log file and then terminate the
                // execution of this method.
                if (!Directory.Exists(baseDir))
                {
                    // The folder, 'baseDir', does NOT exist on the file system.  This is not
                    // desirable.
                    System.Diagnostics.Debug.WriteLine(
        ...
        /// <summary>
        /// Attempts to obtain the fully-qualified pathname of the folder that
        /// contains whatever assembly this code is running in.
        /// </summary>
        /// <returns>
        ...
        private static string GetContainingBaseDir()
        {
            ...
            try
            {
                ...
                result = Path.GetDirectoryName(typeof(VsixHosting).Assembly.Location);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Returns a sensible log file path under <c>%PROGRAMDATA%</c> for a
        /// VSIX.
        /// </summary>
        /// <param name="assemblyTitle">
        /// (Required.) A <see cref="T:System.String" />
        /// containing the title of the assembly that is to be logged.
        /// <para />
        ...
        /// the <see cref="F:System.String.Empty" /> value is passed as the argument of
        /// either of the <paramref name="assemblyTitle" /> or
        /// <paramref name="productName" /> parameters, then this method returns the
        /// <see cref="F:System.String.Empty" /> value.
        /// </remarks>
        /// <returns>
        /// If successful, a <see cref="T:System.String" /> containing the
        ...
        )
        {
            ...
            try
            {
                ...
                // Check whether the value of the parameter, 'productName', is blank. If this is so,
                // then emit an error message to the log file, and then terminate the execution of
                // this method.
                if (string.IsNullOrWhiteSpace(productName))
                {
                    // The parameter, 'productName' was either passed a null value, or it is blank.
                    // This is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                // then send an  error to the log file and quit, returning the default value of the
                // result variable.
                if (string.IsNullOrWhiteSpace(baseFolder))
                {
                    // The variable, 'baseFolder', has a null reference for a value, or is blank.
                    // This is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                // Check to see whether the folder, 'baseFolder', exists on the file system. If this
                // is not the case, then write an error message to the log file, and then terminate
                // the execution of this method.
                if (!Directory.Exists(baseFolder))
                {
                    // The folder, 'baseFolder', does NOT exist on the file system.  This is not
                    // desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                var folder = Path.Combine(baseFolder, $@"xyLOGIX, LLC\{productName}\Logs");
        ...
        private static void InitializeCurrentAppDomain()
        {
            try
            {
                ...
                // Check to see if the required property, 'AppDomain.CurrentDomain', has a null
                ...
                {
                    // The property, 'AppDomain.CurrentDomain', has a null reference for a value.
                    // This is not desirable.
                    System.Diagnostics.Debug.WriteLine(
        ...
        /// <summary>
        /// Determines whether this code is running inside Visual Studio (i.e.,
        /// the host process' name is <c>devenv.exe</c>) or not.
        /// <para />
        ...
        internal static bool IsVsixHost()
        {
            ...
            try
            {
                ...
                // Check to see if the variable, 'proc', has a null reference for a value. If it
                // does, then emit an error to the Debug output, and terminate the execution of this
                // method, returning the default return value.
                if (proc == null)
                {
                    // The variable, 'proc', has a null reference for a value.  This is not
                    // desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                {
                    // The property, 'proc.ProcessName', appears to have a null or blank value. This
                    // is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                result = SpecialVisualStudioProcessNames.DevEnv.Equals(
                    proc.ProcessName, StringComparison.OrdinalIgnoreCase
                );
            }
            catch (Exception ex)
            {
        ...
        /// raised by the current <c>AppDomain</c> when it fails to resolve an assembly.
        /// </summary>
        /// <param name="sender">
        ...
        )
        {
            ...
            try
            {
                ...
                // Check to see if the required parameter, 'e', is null. If it is, then write an
                // error message to the log file and then terminate the execution of this method,
                // returning the default return value.
                if (e == null)
                {
                    // The method parameter, 'e', is required and is not supposed to have a NULL
                ...
                // Check to see if the variable, 'targetAssemblyName', has a null reference for a
                ...
                {
                    // The variable, 'targetAssemblyName', has a null reference for a value.  This
                    // is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                // We can use the variable, 'targetAssemblyName', because it's not set to a null
                // reference.
                System.Diagnostics.Debug.WriteLine(
                ...
                // Check to see if the required property, 'targetAssemblyName.Name', appears to have
                // a null or blank value. If it does, then send an error to the log file and quit,
                // returning the default value of the result variable.
                if (string.IsNullOrWhiteSpace(targetAssemblyName.Name))
                {
                    // The property, 'targetAssemblyName.Name', appears to have a null or blank
                ...
                if (string.IsNullOrWhiteSpace(baseDir))
                {
                    // The variable, 'baseDir', has a null reference for a value, or is blank.  This
                    // is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                if (!Directory.Exists(baseDir))
                {
                    // The folder, 'baseDir', could not be located on the file system.  This is not
                    // desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                var path = Path.Combine(GetContainingBaseDir(), targetAssemblyName.Name + ".dll");

                System.Diagnostics.Debug.WriteLine(
                    "VsixHosting.OnAssemblyResolve: Checking whether the variable, 'path', has a null reference for a value, or is blank..."
                );

                // Check to see if the required variable, 'path', is null or blank. If it is, then
                // send an  error to the log file and quit, returning the default value of the
                // result variable.
                if (string.IsNullOrWhiteSpace(path))
                {
                    // The variable, 'path', has a null reference for a value, or is blank.  This is
                    // not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                // Check to see whether the target assembly .dll file could be located on the file
                // system. If this is not the case, then write an error message to the log file, and
                // then terminate the execution of this method.
                if (!File.Exists(path))
                {
                    // The target assembly .dll file could NOT be located on the file system.  This
                    // is not desirable.
                    System.Diagnostics.Debug.WriteLine(
        ...
        /// <summary>
        /// Attempts to dispose of the Windows <c>HPROCESS</c> handle for the
        /// specified <paramref name="process" /> object.
        /// </summary>
        /// <param name="process">
        /// (Required.) Reference to an instance of
        /// <see cref="T:System.Diagnostics.Process" /> that is to be disposed.
        /// </param>
        /// <remarks>
        /// If a <see langword="null" /> reference is passed for the argument of
        /// the <paramref name="process" /> parameter, then this method does nothing.
        /// </remarks>
        private static void TryDisposeProcess([NotLogged] Process process)
        {
            try
            {
                System.Diagnostics.Debug.WriteLine(
                    "VsixHosting.TryDisposeProcess: Checking whether the method parameter, 'process', has a null reference for a value..."
                );

                // Check to see if the required parameter, process, is null. If it is, then write an
                // error message to the log file and then terminate the execution of this method.
                if (process == null)
                {
                    // The method parameter, 'process', is required and is not supposed to have a
                    // NULL value.  It does, and this is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                        "VsixHosting.TryDisposeProcess: *** ERROR *** A null reference was passed for the method parameter, 'process'.  Stopping..."
                    );
                ...
                System.Diagnostics.Debug.WriteLine(
                    "VsixHosting.TryDisposeProcess: *** SUCCESS *** We have been passed a valid object reference for the method parameter, 'process.  Proceeding..."
                );
        ...
        /// <summary>
        /// Gets the fully-qualified pathname of the VSIX-local log4net config
        /// file, e.g., <c>log4net.vsix.config</c>, or <c>log4net.config</c>, if present in
        /// the same folder as which the <c>*.dll</c> file containing this code also lives.
        /// </summary>
        /// <remarks>
        ...
        internal static string TryGetLog4NetConfigPath()
        {
            ...
            try
            {
                ...
                // Check to see if the required variable, 'folder', is null or blank. If it is, then
                // send an  error to the log file and quit, returning the default value  of the
                // result variable.
                if (string.IsNullOrWhiteSpace(folder))
                {
                    // The variable, 'folder', has a null reference for a value, or is blank.  This
                    // is not desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                // Check to see whether the folder, 'folder', exists on the file system. If this is
                // not the case, then write an error message to the log file, and then terminate the
                // execution of this method.
                if (!Directory.Exists(folder))
                {
                    // The folder, 'folder', could NOT be located on the file system.  This is not
                    // desirable.
                    System.Diagnostics.Debug.WriteLine(
                ...
                foreach (var candidate in candidates)
                {
                    ...
                    // Check to see if the required variable, 'candidate', appears to have a null or
                    // blank value. If it does, then send an error to the log file and then  skip to
                    // the next loop iteration.
                    if (string.IsNullOrWhiteSpace(candidate))
                    {
                        // The variable, 'candidate', appears to have a null or blank value.  This
                        // is not desirable.
                        System.Diagnostics.Debug.WriteLine(
                    ...
                    // Check to see whether the file, 'file', exists on the file system. If this is
                    // not the case, then write an error message to the Debug output, and then skip
                    // to the next loop iteration.
                    if (!File.Exists(candidate))
                    {
                        // The file, 'file', could NOT be located on the file system.  This is not
                        // desirable.
                        System.Diagnostics.Debug.WriteLine(
